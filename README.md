...
- **Form dropdowns:** rounded outlined fields and rounded menus match the other inputs. The selected option has a soft green highlight and checkmark. Labels wrap at larger text sizes; Android still handles selection and scrolling. Choice and Automatic/Yes/No values keep their existing meaning.
...
This version creates, edits, and deletes activities and reads shared data. Pending activity copies can be corrected or discarded on the phone. Manage children and upload/view photos through the Baby Buddy web interface. There are no widgets, biometric app lock, or scheduled background sync. It is an independent client, not an official Baby Buddy or Baby Buddy Companion release.

Activity age labels use completed minutes below one hour, hours and minutes from one hour to under 24 hours (such as 18h 40m ago), then completed days. Exact whole hours retain 0m; ages are not rounded up.

**Timer notifications:** on supported Android 16+ devices, timers started on this phone request a Live Update chip showing elapsed time directly in the status bar. Timers started by another caregiver retain ordinary notifications. Android controls availability and appearance; disabling promotion falls back to the ordinary notification. Dismissing a local timer notification prevents promotion again for that timer, including after restarting the app; a new timer is eligible again. A quiet ongoing notification shows elapsed time and opens Today for the timer's child. Android renders the counter using the saved start time, so no ticking background service is needed. The notification is removed after a local Stop/Cancel or a sync confirming completion elsewhere. Remote changes are discovered by the existing foreground sync; notifications do not add background polling. Saved timers restore their notifications after app restart, reboot, or an app update. Android 13+ requests permission when a real timer is first available; refusal leaves logging fully usable. Settings → Timer notifications opens Android's notification controls. Android may allow dismissal or suppress notifications; neither action stops the saved timer. Force-stopping the app can prevent notifications/restoration until it is opened again. Lock-screen public content omits child and activity details; visibility follows the user's Android settings.

**Synced deletion:** open an activity's details and choose Delete activity, then confirm its removal for all caregivers. The request is saved offline against the original record ID. Before deleting, sync compares the current server record with the reviewed copy. A changed record requires review; an already-removed record clears the request. A lost reply or interrupted deletion is never automatically retried. Pending deletion cards remain accessible, and totals use confirmed server data until reconciliation. Discard deletion request stops further attempts but cannot restore an activity already removed on the server. An existing pending edit/deletion must be reviewed before another change can be queued for the same activity. Read and delete are separate server calls, so this is not an atomic cross-device lock. Deletion has no Undo.
...
Downloading the APK through Proton Drive or another file-transfer service does not change installation behavior. Open the new APK and accept Android's **Update** prompt; do not uninstall first. An in-place update preserves settings, the saved connection, cached records, and pending entries. The application ID and signing certificate must match, and the internal `versionCode` must be equal or higher. Reinstalling the same version is allowed; each delivered build gets an increased code and a distinct filename to make downloads easy to identify. Version 0.1.21 uses code 22. The update rules below apply only when the application ID matches; 0.1.10 cannot replace pre-publication prototypes.

Test APKs use Android debug signing. Updating an existing installation requires the **same debug key**, including after moving to another computer. The 0.1.19/0.1.20 new-key test APKs have valid debug signatures but cannot update installations signed with the earlier computer's key. An unsigned release APK is not installable. See [Android's update requirements](https://developer.android.com/google/play/app-updates).

On 2026-09-22 the maintainer authorized a fresh installation with the new computer's debug key. Reconnect to the existing server afterward; synced records remain on the server, while uninstalling removes local settings and unsynced work. Preserve this new key for subsequent updates. To compile a debug-signed phone APK, keep the key outside the repository and run:

~~~powershell
.\scripts\build-phone-apk.ps1 -DebugKeystore 'C:\private\signing\debug.keystore'
~~~

This compiles assembleDebug, checks the supplied key against the selected phone-build certificate **before building**, then verifies the generated APK signature. A missing or different key stops the command; it never generates a replacement key or changes the app identity. Output is app/build/outputs/apk/debug/app-debug.apk. The standard debug alias/passwords are used. No signing secrets belong in Git.

The expected public fingerprint defaults to the new-key certificate used by 0.1.19/0.1.20 and selected for the fresh installation. The earlier certificate cannot be recovered from its fingerprint or an APK; it requires the original private keystore. For a separate installation that intentionally uses another key, supply -ExpectedSignerSha256 with its independently verified certificate fingerprint. Do not override this merely to silence a mismatch for an installed app. For direct Gradle/Android Studio builds, BBP_DEBUG_KEYSTORE can select the preserved debug keystore; the phone helper also verifies certificate continuity. Ordinary development builds still use the machine's default debug key when this variable is unset.
